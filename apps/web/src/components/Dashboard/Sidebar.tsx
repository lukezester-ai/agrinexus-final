		...
		<div className="flex flex-col gap-0.5">
			...
			{items.map((item) => (
				...
				>
					...
					{item.badge && (
						<span className="ms-auto rounded-full bg-forest-700/10 px-1.5 py-px font-mono text-[9px] font-medium text-forest-700">
							{item.badge}
						</span>
					)}
				</Link>
...
export default function Sidebar({
	...
	const sideItems = {
		daily: [
			{ icon: "◎", label: c.radar, href: "/dashboard", active: pathname === "/dashboard" || pathname === "/" },
			...
			{ icon: "💬", label: c.ask, href: "/dashboard/ask", active: pathname === "/dashboard/ask" },
		...
		more: [
			{ icon: "👥", label: c.community, href: "/community", active: pathname === "/community" },
			{ icon: "⚙", label: c.settings, href: "/dashboard/settings", active: pathname === "/dashboard/settings" },
		],
	};
