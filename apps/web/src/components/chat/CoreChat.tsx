...
const copy = {
	en: {
		title: "Ask",
		sub: "Assistant for this organization and the matching workflow.",
		placeholder: "Ask about your organization or matching workflow…",
		send: "Send",
		sending: "Thinking…",
		empty: "Start a conversation.",
		agent: "Assistant",
		offline: "Chat needs MISTRAL_API_KEY on the server.",
		err: "Could not reach chat. Try again.",
	},
	bg: {
		title: "Питай",
		sub: "Асистент за организацията и за съпоставянето.",
		placeholder: "Питай за организацията или за съпоставянето…",
		send: "Изпрати",
		sending: "Мисля…",
		empty: "Започни разговор.",
		agent: "Асистент",
		offline: "Чатът изисква MISTRAL_API_KEY на сървъра.",
...
export function CoreChat({ locale, compact, mobileFill }: Props) {
	...
	const send = useCallback(async () => {
		...
		try {
			...
			const reply = typeof data.response === "string" && data.response ? data.response : data.error || c.err;
			setMessages((prev) => [
				...prev,
				{ id: newId(), role: "assistant", text: reply, agent: data.handledBy, traceId: data.traceId },
			]);
		} catch {
			setMessages((prev) => [...prev, { id: newId(), role: "assistant", text: c.err }]);
		} finally {
			setLoading(false);
		}
		...
		>
			{!compact ? (
				<div className="border-b border-ink/[0.06] px-5 py-4">
					...
					{configured === false ? <p className="mt-2 text-[11px] text-amber-800/90">{c.offline}</p> : null}
				</div>
			) : null}
			<div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-[200px]">
				{messages.length === 0 && !loading ? <p className="text-sm text-ink/45 text-center py-8">{c.empty}</p> : null}
				{messages.map((m) => (
					<div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
						<div
							className={`max-w-[88%] rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed ${
								m.role === "user" ? "bg-forest-700 text-white" : "bg-ink/[0.05] text-ink border border-ink/[0.06]"
							}`}
						>
							{m.agent ? (
							...
							{m.role === "assistant" && m.traceId && <ChatFeedback traceId={m.traceId} />}
						</div>
					</div>
				))}
				{loading ? <div className="text-xs text-ink/45 animate-pulse px-2">{c.sending}</div> : null}
				<div ref={bottomRef} />
			</div>
			<form
