...
- **Market quotes** in the mesh and Academy use **Yahoo Finance** (delayed); the LLM must **not invent** prices when the snapshot is present (see `verticals/agriculture/market-data/market-snapshot.ts` and `verticals/agriculture/ai/api-agrinexus-policy.ts`).
