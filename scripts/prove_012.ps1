# Apply 001-012 on a clean Postgres 16 and run the RLS + engine + introduction + relationship + radar suite.
...
docker exec $container pg_isready -U postgres -d $db | Out-Null
if ($LASTEXITCODE -ne 0) { throw "Postgres in $container left ready state" }
